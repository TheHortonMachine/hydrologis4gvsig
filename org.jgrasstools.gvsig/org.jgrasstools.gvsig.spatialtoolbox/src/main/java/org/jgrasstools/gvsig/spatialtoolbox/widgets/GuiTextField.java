        ...
        if (isFile) {
            ...
            browseButton.addActionListener(new ActionListener(){
                public void actionPerformed( ActionEvent e ) {
                    File[] files;
                    if (isInFile) {
                        files = dialogManager.showOpenFileDialog("Open", JGTUtilities.getLastFile());
                    } else {
                        files = dialogManager.showSaveFileDialog("Save", JGTUtilities.getLastFile());
                    }
        ...
        if (isFolder) {
            ...
            browseButton.addActionListener(new ActionListener(){
                public void actionPerformed( ActionEvent e ) {
                    File[] files;
                    if (isInFolder) {
                        files = dialogManager.showOpenDirectoryDialog("Open", JGTUtilities.getLastFile());
                    } else {
                        // there is no folder save
                        files = dialogManager.showOpenDirectoryDialog("Save", JGTUtilities.getLastFile());
                    }
                    if (files != null && files.length > 0) {
                        String absolutePath = files[0].getAbsolutePath();
                        JGTUtilities.setLastPath(absolutePath);
